...
Bx = np.array(fld.arBx).reshape((Nx, Ny, Nz), order='C')
By = np.array(fld.arBy).reshape((Nx, Ny, Nz), order='C')
Bz = np.array(fld.arBz).reshape((Nx, Ny, Nz), order='C')

x = np.linspace(0, fld.rx, Nx)
y = np.linspace(0, fld.ry, Ny)
z = np.linspace(0, fld.rz, Nz)
...
surf = ax.plot_surface(X, Y, Bz[:,:,k], cmap='viridis', linewidth=0)
ax.set_xlabel("x [m]")
ax.set_ylabel("y [m]")
ax.set_zlabel("Bz [T]")
ax.set_title(f"Bz at z = {z[k]:.4f} m")
