...
partTrajLstY = []
yp_shift_list = np.linspace(1e-5, -1e-5, 11)
...
import matplotlib
norm = matplotlib.colors.Normalize(vmin=yp_shift_list.min()*1000, vmax=yp_shift_list.max()*1000)
cmap = matplotlib.cm.viridis

line_color = cmap(norm(yp_shift_list[0]*1e3))

fig = plt.figure(figsize=(8,6))
ax = fig.subplots(2,1)
partTrajRef = partTrajLstY[len(partTrajLstY)//2]
for i, partTraj in enumerate(partTrajLstY):
    line_color = cmap(norm(yp_shift_list[i]*1e3))
...
sm.set_array([])
cbar = plt.colorbar(sm, ax=ax, label='Initial offset momentum y\'')
plt.show()
...
for i, partTraj in enumerate(partTrajLstY):
    line_color = cmap(norm(yp_shift_list[i]*1e3))
    ax.plot(ctMesh, partTraj.arY, color = line_color, linestyle="-", marker="None")
ax.set_xlabel('ct [m]')
ax.set_ylabel('Vertical Position  [mm]')
ax.set_title('Electron Trajectories in Vertical Plane')
sm = matplotlib.cm.ScalarMappable(cmap=cmap, norm=norm)
sm.set_array([])
cbar = plt.colorbar(sm, ax=ax, label='Initial offset momentum y\'')
plt.show()
