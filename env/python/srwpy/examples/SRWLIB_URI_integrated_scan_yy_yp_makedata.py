...
class SRWSimulationEngine:
    def __init__(self, mag_field_dir, Lgap_list):
        self.loader = UndulatorField(mag_field_dir)
        self.mag_cache = {lgap: self.loader.configure_double_undulator_fld(Lgap=lgap) for lgap in Lgap_list}
        #os.makedirs(self.output, exist_ok=True)
    def calculate_and_save(self, params):
        y, yp, lgap, screen_y = params
        # 1. load from mag fld cache
        magFldCnt = self.mag_cache[lgap]
        # 2. run radiation calc
        intensity_data = self._run_srw_calculation(magFldCnt, y, yp, screen_y)
        # 3. save results
        filename = f"{y*1e3:2f}mm_{yp*1e3:3f}mrad_{screen_y*1e3:3f}mm.txt" ### include . in file name
        np.savetxt(self.output_dir + filename, intensity_data)
        return filename
    def _run_srw_calc(self, magFldCnt, y, yp, screen_y):
        return

    def elerton_initialization(self):
        ...
        elecBeam.partStatMom1.gamma = .195/0.51099890221e-03 #Relative Energy
        return elecBeam

    def calclate_wavefront_radiation(self):
        wfr = SRWLWfr()
        wfr.allocate(_ne=1, _nx=11, _ny=11) #Numbers of points vs Photon Energy
        magFldCnt = self.magFldCnt
...
class SRWSimulation:
    def __init__(self, mafFldCnt, elecBeam):
        self.magFldCnt = mafFldCnt

        self.elecBeam = elecBeam

        self.Screen_x_config = [-3.e-3, 3.e-3, 3] #m
        self.Screen_y_config = [-2.2e-3, -1.3e-3, 31] #m
        self.lambda_L_obs = 404. #nm
        self.screen_z = 14. #m
        self.npTraj = 50001 #Number of Points for Trajectory calculation

    def run_radiation_calc(self):
        Intensity_list = []
        for Lgap in self.Lgap_arange:
    ...
    def generate_filename(self, beam_y, beam_yp, screen_y_min):
        return f"BeamY_{beam_y*1e3:.3f}mm_BeamYP_{beam_yp*1e3:.3f}mrad_ScreenYmin_{screen_y_min*1e3:.3f}mm.txt"
    
    def simulate

    def save_raw_intensity(self, beam_y, beam_yp, intensity_array, filename):
        return 0
    

class UndulatorField():
    def __init__(self,MagFiledDirPath):
        self.dirPath = MagFiledDirPath
    def configure_double_undulator_fld(self, Lgap=0.860):
        B_z = np.loadtxt(self.dirPath +"Bz_profile.txt", dtype='float')
        B_y = np.loadtxt(self.dirPath+"By_profile.txt", dtype='float')
        z   = np.loadtxt(self.dirPath+"z_positions.txt", dtype='float')
        y   = np.loadtxt(self.dirPath+"y_positions.txt", dtype='float')

        n_y, n_z = B_y.shape
        n_x = 1
        self.Bx_arr = array('d', [0.]*(n_x*n_y*n_z))
        self.By_arr = array('d', [0.]*(n_x*n_y*n_z))
        self.Bz_arr = array('d', [0.]*(n_x*n_y*n_z))
        for iz in range(n_z):
            for iy in range(n_y):
                for ix in range(n_x):
                    idx = ix + iy*n_x + iz*n_x*n_y
                    self.By_arr[idx] = B_y[iy][iz]*1e-3  #Convert from mT to T
                    self.Bz_arr[idx] = B_z[iy][iz]*1e-3  #Convert from mT to T 
        magFld = SRWLMagFld3D( self.Bx_arr, self.By_arr, self.Bz_arr, n_x, n_y, n_z, 1e-3, y[-1] - y[0], z[-1] - z[0], 1 )
...
if __name__ == "__main__":
    # パラメータのリストを作成 (すべての組み合わせ)
    # [(y, yp, lgap, screen_z), ...]
    tasks = [(y, yp, lgap, sz) for y in y_vals for yp in yp_vals for lgap in lgap_vals for sz in screen_vals]
    
    engine = SRWSimulationEngine(mag_dir, lgap_vals)
    
    # プロセス並列で一気に実行
    with ProcessPoolExecutor() as executor:
        results = list(executor.map(engine.calculate_and_save, tasks))
    
    print(f"全 {len(results)} タスクが完了しました")
    



#**********************Defining Study Parameters:
Loop_beam_y_lst = np.linspace(0.e-3, 0.50e-3, 10) # m
Loop_beam_yp_lst = np.linspace(-0.18e-3, -0.09e-3, 10) # m

#**********************Magnetic Field from Text Files:
MagFldLoader = UndulatorField("/home/nishi/Mainz/mainz/BeamEnergyCalib/2024Calib/MagField/")
magFld = MagFldLoader.configure_double_undulator_fld(Lgap=0.860)
...
result_lst =[]
for y_i, beam_yp_val in enumerate(beam_yp_lst):
    beam_x, beam_y, beam_xp, beam_yp = 0., 0.25e-3, 0., beam_yp_val
    lambda_L_obs = 404. #nm
    screen_z = 14. #m
    print("========================================================")
    ...
    for cut_cond_i in range(len(cut_config)):
        ...
        ax[0][y_i].set_title("y' = {:.3f} mm ".format(beam_yp_val*1.e3))
        ...
        ax_ext_val[0][y_i].set_title("y' = {:.3f} mm ".format(beam_yp_val*1.e3))
    ...
    ax_track[0][y_i].set_title("y' = {:.3f} mm ".format(beam_yp_val*1.e3))

fig.suptitle("Fitted Oscillation Parameters vs Undulator Position @ y = {:.3f} mm".format(beam_y*1.e3))
fig_ext_val.suptitle("Fitted Oscillation Parameters vs Undulator Position @ y = {:.3f} mm".format(beam_y*1.e3))
fig_track.suptitle("Electron Trajectory, Ydiff = Ydown - Yup @ y = {:.3f} mm".format(beam_y*1.e3))
...
np.savetxt(SaveDirPath+RunNum+"_Screen_yp_values.txt", screen_y_range)
np.savetxt(SaveDirPath+RunNum+"_Beam_yp_values.txt", beam_yp_lst)
...
data = data.reshape(len(beam_yp_lst), 6, 6)
...
ax1.plot(beam_yp_lst, val1_m[:, 0], "o-", label=r"$\lambda_{osc}$")
ax1.plot(beam_yp_lst, val2_m[:, 0], "s-", label=r"$\Phi$")
...
ax2.plot(beam_yp_lst, val1_m[:, 1], "o-",  color ="tab:blue", label="lam (up)")
ax2.plot(beam_yp_lst, val1_m[:, 5], "o--", color ="tab:blue",  label="lam (down)")
ax2.plot(beam_yp_lst, val2_m[:, 1], "s-",  color ="tab:orange",label="phi (up)")
ax2.plot(beam_yp_lst, val2_m[:, 5], "s--", color ="tab:orange",label="val2 (down)")
...
ax3.plot(beam_yp_lst, val1_m[:, 1] - val1_m[:, 5], "o-", label="lam")
ax3.plot(beam_yp_lst, val2_m[:, 1] - val2_m[:, 5], "s-", label="phi")
...
ax4.plot(beam_yp_lst, (val1_m[:, 1] + val1_m[:, 5])/2., "o-",color ="tab:blue", label="lam mean")
ax4.plot(beam_yp_lst, val1_m[:, 3], "^-", color ="tab:blue", label="lam center")
ax4.plot(beam_yp_lst, (val2_m[:, 1] + val2_m[:, 5])/2., "s-", color ="tab:orange", label="phi mean")
ax4.plot(beam_yp_lst, val2_m[:, 3], "v-", color ="tab:orange", label="phi center")
